...
constexpr charls::interleave_mode get_interleave_mode(std::string_view mode) {
  if (mode == "none") {
...
std::vector<uint8_t> interleaved_to_planar(
...
PYBIND11_MODULE(_pycharls, module) {
  ...
  module.def("encode",
    ...
    auto interleave_mode = kwargs.contains("interleave") ?
        get_interleave_mode(kwargs["interleave"].cast<std::string>()) : charls::interleave_mode::sample;
    ...
    if (interleave_mode == charls::interleave_mode::none) {
      std::vector<uint8_t> src = interleaved_to_planar(static_cast<const uint8_t * >(src_buffer_info.ptr), frame_info);
